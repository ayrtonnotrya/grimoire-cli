...
**Note:** PDF files up to 50MB or 1000 pages are supported. This limit applies
to both inline data and Files API uploads.
...
reference the file in subsequent requests. We recommend using the [Files API](/gemini-api/docs/document-processing#large-pdfs) for larger documents that you need to refer to in multi-turn interactions to improve request latency and reduce bandwidth usage.
